...
import DeleteConfirmModal from "../../../../Shared/ConfirmationModel/ConfirmationModel";
...
export default function GroupList() {
  ...
  const paginatedData = groupList?.slice(
    startIndex,
  ...
  const handleUpdateCurrentGroup = useCallback(
    async function (dataInfo: CreateGroupProps, groupId: string) {
  ...
  const createNewGroup = useCallback(
    async function (dataInfo: CreateGroupProps) {
      const toastId = toast.loading("Waiting.....");
      setErrorMessage(null);
  ...
  const handleDeletGroup = useCallback(
    async function () {
      const toastId = toast.loading("Waiting.....");
      setErrorMessage(null);
      setIsLoading(true);
      try {
        const { data } = await axiosInstance.delete(
          GROUP.DELETE_GROUP(selectGroupId!)
        );
  ...
  return (
    <div className="group-list-container">
      ...
      <div className="add-btn text-end">
        <button onClick={handleShowAddAndUpdateModel} className="addBtn">
          {" "}
          <i className="fa-solid fa-circle-plus"></i> Add Group
        </button>
      </div>
      ...
      <div className="group-list-box rounded-2xl px-4 border-[1px] border-[#f3f4f6] mt-4 py-6">
        ...
        {paginatedData.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {paginatedData?.map((group: Group) => (
              <GroupListCard
                onDelete={(groupId: string) => {
                  setSelectedGroupId(groupId);
                  setShowDeletMessage(true);
                }}
                onUpdate={(currentGroup: Group) => {
        ...
        <Pagination
          ...
          totalItems={groupList?.length || 0}
        />
      </div>

      {/* add and update form pop up */}

      {showAddAndUpdateCard && (
        <GroupAddAndUpdateCard
