...
import type {
  CreateGroupProps,
  GroupAddAndUpdateProps,
  GroupStudentProps,
} from "../../../../../Interfaces/Groups.interface";

export default function GroupAddAndUpdateCard({
  errorMessage,
  onClose,
  onCreate,
  title,
  ...
  useEffect(() => {
    async function fetchStudents() {
      ...
      try {
        ...
        const { data: unassignedStudents } = await axiosInstance.get(
        ...
        if (title === "Update Current Group" && currentGroup?._id) {
          const { data: groupDetails } = await axiosInstance.get(
            GROUP.GET_BY_ID(currentGroup._id)
          );
      ...
      <div className="form-box bg-white w-full sm:w-[40%] pb-5">
        <form
          onSubmit={handleSubmit((data) => {
            ...
            if (title === "Update Current Group" && currentGroup)
              onUpdate(data, currentGroup?._id);
          })}
        >
          <div className="form-header flex justify-between border-[1px] border-main-border-color items-center">
            <h2 className="px-3 font-bold capitalize text-gray-600 text-2xl">
              {title}
            </h2>
            <div className="act-btn flex">
              <button
                ...
                type="submit"
                className="true-btn p-3 border-l-[1px] border-main-border-color"
              >
              ...
              <button
                ...
                disabled={isSubmitting}
                type="button"
                className="false-btn p-3  border-l-[1px] border-main-border-color"
              >
          ...
          <div className="form-inputs px-3 py-6">
            ...
            <div className="group-name border-[1px] border-main-border-color rounded-2xl flex overflow-hidden mb-1">
              <label className="bg-[#ffeddf] p-3" htmlFor="groupName">
                Group Name
              </label>
              <input
                {...register("name", GROUP_MODULE.NAME)}
                className="border-0 outline-0 grow"
                type="text"
                id="groupName"
              />
            </div>
            ...
            {isLoading ? (
              <div className="flex justify-center items-center">
                <ImSpinner4 className="text-3xl animate-spin" />
              </div>
              ...
              <p className=" text-red-700 capitalize text-center">
                {" "}
                No Students Available Now
              </p>
            )}
          </div>
          {errors.students && (
            <p className="error-message mb-3 ml-3">{errors.students.message}</p>
          )}

          {errorMessage && (
            <p className="error-message text-black text-center mb-3 ml-3">
              {errorMessage}
            </p>
          )}
        </form>
